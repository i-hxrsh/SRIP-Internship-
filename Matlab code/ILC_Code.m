...
C=[1 0 0 0;0 1 0 0];
forward_kinematics=[2*lc1 2*lc2; 2*lc1 2*lc2];
[~,n]=size(A);
[l,m]=size(C*B);
...
alpha=0.0175;% VL Perturbation level
...
%         y_=y;
    ...
    for k1=1:l
        y_dash_ref=[yref(1); yref(1)+yref(2)];
        y_dash=[y(1); y(1)+y(2)];
        E(k1,k)=(norm(abs(forward_kinematics*cos(y_dash_ref(k1,1:(time/ts)))-forward_kinematics*cos(y_dash(k1,1:(time/ts))))).^2);
    end
    if norme==0
        E_(:,k)=norm(forward_kinematics*cos(y_dash_ref(:,1:(time/ts)))-forward_kinematics*cos(y_ref(:,1:(time/ts))));
    end
    if norme==1
        E_(:,k)=norm(forward_kinematics*cos(y_dash_ref(:,1:(time/ts)))-forward_kinematics*cos(y_dash(:,1:(time/ts))))/norm(forward_kinematics*cos(y_dash_ref(:,1:(time/ts))));
    end
    if (k==t1)
        y1=y;
        x1=x;
...
plot(1:(t2-t1),((E_(t1+1:t2))),'m','LineWidth',1.5)
plot(1:(t3-t2),((E_(t2+1:t3))),'g--','LineWidth',2)
plot(1:num_trials*1,((E_(t3+1:t4))),'b','LineWidth',1.5)
...
plot(normuFF,'LineWidth',0.5);hold on
