...
forward_kinematics=[2*lc1 2*lc2];
...
nx=0.01;% Noise levels
ny=0.01;% Noise levels
%% Direction of reaching task
xi_temp=0;
yi_temp=0;
D=(xi_temp^2+yi_temp^2-4*lc1^2-4*lc2^2)/(8*lc1*lc2);
yi=acosd(D);
xi=90-atan2d(2*lc2*sin(acos(D)),(2*lc1+2*lc2*D));
yi=deg2rad(yi);
xi=deg2rad(xi);
2*lc1*cos(xi)+2*lc2*cos(xi+yi)
2*lc1*sin(xi)+2*lc2*sin(xi+yi)
xf_temp=0;

yf_temp=0.25;
D_=(xf_temp^2+yf_temp^2-(2*lc1)^2-(2*lc2)^2)/(8*lc1*lc2);
yf=((atan2d(sqrt(1-D_^2),D_)));
xf=atan2d(yf_temp,xf_temp)-atan2d((2*lc2*(sqrt(1-D_^2))),(2*lc1+2*lc2*D_));
yf=deg2rad(yf);
xf=deg2rad(xf);
xfx=xf;
2*lc1*cos(xf)+2*lc2*cos(xf+yf)
2*lc1*sin(xf)+2*lc2*sin(xf+yf)
...
count=5;
for lambda_ILC=[1,0.998,0.995,0.997,0.996,0.993]
    count=count-1;
%% Init. variables
norme=0;
plot_surf=0;
...
for k=1:paradigm_trials
    ...
    %% Auxilliary steps
    for k1=1:l
        y_dash_ref=[forward_kinematics*cos([1 0; 1 1]*yref);forward_kinematics*sin([1 0; 1 1]*yref)];
        y_dash=[forward_kinematics*cos([1 0; 1 1]*y);forward_kinematics*sin([1 0; 1 1]*y)];
%         y_dash_ref=yref;
%         y_dash=y;
        E(k1,k)=(norm(abs(y_dash_ref(k1,1:(time/ts))-y_dash(k1,1:(time/ts)))).^2);
    end
    if norme==0
        E_(:,k)=norm(y_dash_ref(:,1:(time/ts))-y_dash(:,1:(time/ts)));
    end
    if norme==1
        E_(:,k)=norm(y_dash_ref(:,1:(time/ts))-y_dash(:,1:(time/ts)))/norm(y_dash_ref(:,1:(time/ts)));
    end
    if (k==t1)
        y1=y;
        x1=x;
    ...
    if (k<=(2*num_trials)) %(Baseline)
        if (rem(k,20)==0)||(k==(2*num_trials+1))
            ...
            subplot(1,4,1)
            hold on
            plot(y_dash(1,1:end-2),y_dash(2,1:end-2),'b-','LineWidth',0.1)
            %             axis equal
            xlabel('X (m)')
            ylabel('Y (m)')
            title('Baseline')
...
title('\lambda_I_L_C = '+string(lambda_ILC))
hold off
end
if count==3
subplot(2,3,2)
